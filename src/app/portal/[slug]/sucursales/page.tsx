...
export default async function PortalSucursalesPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  // Ficha exclusiva del admin del cliente o de staff INGEGAR viendo el
  // portal — un usuario de sucursal normal no debe llegar acá.
  const isStaff = isStaffViewing(session)
  const isClientAdmin = session?.user?.role === 'client' && !!session?.user?.isClientAdmin
  if (!isStaff && !isClientAdmin) redirect(`/portal/${slug}/dashboard`)
