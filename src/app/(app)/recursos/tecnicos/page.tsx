        ...
        <>
          ...
          <div className="mt-5 grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-3">
            {active.map((t) => {
              ...
              const checklist = mandatoryDocChecklist(t.documents)
              const docsOk = checklist.filter((c) => c.complete).length
              ...
              return (
                ...
                >
                  <div className="flex items-start gap-3">
                    <div className={`flex h-11 w-11 shrink-0 items-center justify-center rounded-lg text-sm font-bold ${CONTRACT_TYPE_BADGE[contractType]}`}>
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="mt-0.5 flex flex-wrap items-center gap-1">
                        <span className={`rounded-full px-2 py-0.5 text-[11px] font-medium ${CONTRACT_TYPE_BADGE[contractType]}`}>
                          {CONTRACT_TYPE_LABELS[contractType]}
                        </span>
                        {(contractExpired || contractWarn) && (
                          <span className={`inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-[11px] font-medium ${contractExpired ? 'bg-red-50 text-red-700' : 'bg-amber-50 text-amber-700'}`}>
                            <span className={`h-1.5 w-1.5 rounded-full ${contractExpired ? 'bg-red-500' : 'bg-amber-500'}`} />
                            {contractExpired ? 'Contrato vencido' : `Vence en ${contractDays}d`}
                          </span>
                        )}
                      </div>
                    </div>
                    {t.phone && (
                      <div className="flex shrink-0 gap-1">
                        <a
                          ...
                          className="flex h-9 w-9 items-center justify-center rounded-full bg-gray-100 text-gray-600 transition hover:bg-gray-200 active:bg-gray-300"
                        >
                        ...
                        {waPhone && (
                          <a
                            ...
                            className="flex h-9 w-9 items-center justify-center rounded-full bg-green-50 text-green-600 transition hover:bg-green-100 active:bg-green-200"
                          >
                  ...
                  <div className="space-y-0.5 text-xs text-gray-500">
                    <p>
                      Camioneta:{' '}
                      {t.vehicle ? (
                        <Link href={`/recursos/vehiculos/${t.vehicle.id}`} className="font-medium text-gray-700 hover:text-brand hover:underline">
                          {t.vehicle.plate}
                        </Link>
                      ) : (
                  ...
                  <div className="grid grid-cols-3 gap-1.5 border-t border-gray-100 pt-3">
                    ...
                    <StatPill
                      ...
                      label="Docs."
                      tone={docsOk === checklist.length ? 'ok' : 'warn'}
                    />
                  </div>
