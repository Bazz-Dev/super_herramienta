...
import { esc, formatDate } from '@/lib/quotes/format'
...
function baseStyles(): string {
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .masthead { display: flex; justify-content: space-between; align-items: flex-start; gap: 24px; padding-bottom: 14px; border-bottom: 3px solid var(--color-primary); }
  .masthead .tagline { color: var(--color-muted); margin-top: 2px; font-size: 12px; }
  .masthead .web { color: var(--color-muted); font-size: 10px; margin-top: 6px; }
  .masthead .right { text-align: right; }
  .masthead .doc-type { font-size: 18px; font-weight: 700; color: var(--color-black); letter-spacing: 0.04em; }
  .masthead .dept { font-size: 10.5px; color: var(--color-muted); margin-top: 3px; line-height: 1.5; }
  .masthead .version { display: inline-block; margin-top: 6px; padding: 2px 8px; background: var(--color-black); color: #fff; font-size: 9px; letter-spacing: 0.05em; border-radius: 2px; }

  /* ---- Identification card ---- */
  .id-card { margin-top: 18px; border: 1px solid var(--color-border); border-radius: 4px; overflow: hidden; }
  ...
  .id-k { width: 96px; flex-shrink: 0; padding: 7px 10px; background: var(--color-band); font-size: 9px; text-transform: uppercase; letter-spacing: 0.04em; color: var(--color-muted); display: flex; align-items: center; }
  .id-v { padding: 7px 10px; font-size: 12px; color: var(--color-black); font-weight: 600; word-break: break-word; }

  .intro-line { margin-top: 20px; font-size: 13px; font-weight: 600; color: var(--color-black); }
  ...
  .section-header {
    ...
    padding: 8px 14px; font-weight: 600; font-size: 12px; letter-spacing: 0.02em; margin-bottom: 10px;
  }
  .section-header .n { color: var(--color-primary); margin-right: 6px; }
  .section-body { font-size: 12px; }
  .section-body p { margin-bottom: 8px; }
  ul.bullets { list-style: none; margin-top: 4px; }
  ul.bullets li { position: relative; padding: 5px 0 5px 16px; }
  ...
  .photo img { width: 100%; height: 62mm; object-fit: cover; border: 1px solid var(--color-border); display: block; border-radius: 2px; }
  .photo .caption { font-size: 11px; color: var(--color-muted); margin-top: 4px; }

  /* ---- Footer block ---- */
  .doc-footer { margin-top: 30px; padding-top: 12px; border-top: 1px solid var(--color-border); font-size: 10.5px; color: var(--color-muted); line-height: 1.7; }
...
function renderIdCard(data: ReportData): string {
  return `<div class="id-card">
    <div class="id-row">
      ...
      ${idCell('Fecha', formatDate(data.date))}
    </div>
    <div class="id-row">
