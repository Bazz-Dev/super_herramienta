  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <div className="flex flex-wrap items-center gap-2 rounded-lg bg-gray-50 p-2">
        ...
        {columns.length === 0 && <span className="text-xs text-gray-400">ninguna</span>}
        {columns.map((c) => (
          <span key={c.id} className="flex items-center gap-1 rounded-md border border-gray-200 bg-white px-1.5 py-1">
            <input
              value={c.label}
            ...
            <button
              ...
              aria-label={`Quitar columna ${c.label}`}
      ...
      {/* Items table */}
      {items.length === 0 ? (
        <p className="rounded-md bg-gray-50 px-3 py-4 text-center text-xs text-gray-400">
          ...
          <table className="w-full border-collapse text-sm">
            <thead>
              <tr className="bg-gray-50 text-left text-[11px] uppercase tracking-wide text-gray-500">
                ...
                {columns.map((c) => (
                  <th key={c.id} className="px-2 py-2 font-medium">{c.label || '—'}</th>
                ))}
                <th className="px-2 py-2 text-right font-medium">Cant.</th>
                <th className="px-2 py-2 text-right font-medium">Unitario</th>
                ...
                <tr key={i} className="border-t border-gray-100 align-top">
                  ...
                  {columns.map((c) => (
                    <td key={c.id} className="px-2 py-1.5" style={{ minWidth: 90 }}>
                      <TextInput value={item.custom?.[c.id] ?? ''} onChange={(e) => updateCustom(i, c.id, e.target.value)} />
                    </td>
                  ))}
                  ...
                  <td className="px-2 py-1.5 text-right text-xs font-semibold whitespace-nowrap text-ink" style={{ width: 110 }}>
                    {formatMoney(item.quantity * item.unitPrice, currency)}
                  </td>
                  <td className="px-1 py-1.5">
                    <IconButton label="Eliminar ítem" onClick={() => onItemsChange(items.filter((_, idx) => idx !== i))}>
                      <TrashIcon />
                    </IconButton>
                  </td>
                </tr>
