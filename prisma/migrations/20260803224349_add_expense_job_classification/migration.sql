...
CREATE TABLE "new_expenses" (
    ...
    CONSTRAINT "expenses_jobId_fkey" FOREIGN KEY ("jobId") REFERENCES "jobs" ("id") ON DELETE SET NULL ON UPDATE CASCADE,
    CONSTRAINT "expenses_approvedById_fkey" FOREIGN KEY ("approvedById") REFERENCES "users" ("id") ON DELETE SET NULL ON UPDATE CASCADE
);
INSERT INTO "new_expenses" ("amount", "approvedById", "assignmentId", "category", "createdAt", "date", "description", "id", "paidAt", "receiptUrl", "rejectedReason", "status", "technicianId", "tenantId", "ticketId", "updatedAt") SELECT "amount", "approvedById", "assignmentId", "category", "createdAt", "date", "description", "id", "paidAt", "receiptUrl", "rejectedReason", "status", "technicianId", "tenantId", "ticketId", "updatedAt" FROM "expenses";
DROP TABLE "expenses";
