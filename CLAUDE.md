...
- **`TechnicianDocument`** (`type: DocType` — `contrato/carnet/epp/altura/antecedentes/licencia/otro`, `label` libre, `expiryDate` opcional): ya existía; se agregó `carnet`. `MANDATORY_DOC_TYPES` (`labels.ts`) = **`contrato/carnet`** (decisión del dueño — altura/epp/antecedentes/licencia quedan como tipos disponibles pero no "fijos"). `carnet` exige 2 caras (Frontal/Reverso vía `label`, sin duplicar el enum). `mandatoryDocChecklist(docs)` calcula el ✓/✗, usado tanto en la card del listado (`/recursos/tecnicos`) como en la ficha.
- **`doc-section.tsx` — slots fijos + libre**: Contrato y Carnet (Frontal/Reverso) son 3 slots visuales fijos (`FixedSlot`) — vacío = dashed border + subir directo; con archivo = ✓ verde + Ver/Eliminar. Todo lo demás se agrega con "+ Agregar documento" (solo nombre + archivo, sin selector de tipo — se guarda como `type: 'otro'`, el nombre va en `label`) y aparece en "Otros documentos" debajo. Cada documento (fijo o libre) tiene un checkbox de selección para el ZIP.
- **ZIP selectivo**: cada plataforma de proveedor/cliente pide un subconjunto distinto de documentos — `GET /api/technicians/[id]/documents/zip?ids=id1,id2,...` descarga solo lo marcado (todo seleccionado por default); sin `?ids=` descarga todo (usado por el botón rápido "ZIP" de la card del listado, que no tiene espacio para selección).
...
- **Rediseño visual (jul-2026)**: cards de `/recursos/tecnicos` y `/recursos/vehiculos` migradas al mismo lenguaje visual que `/recursos/clientes` (avatar/badge cuadrado + fila de stat-pills + footer simple, sin la barra de color de ancho completo que tenían antes) — la card de técnico suma un stat-pill "Docs." (`x/2`) para ver de un vistazo quién tiene la documentación fija completa. `TechnicianProfileShell` (la ficha) pasó de `max-w-2xl` a `max-w-4xl` con una barra de tabs tipo segmented-control (iconos + fondo gris con la pestaña activa en blanco) en vez de underline tabs planas.
